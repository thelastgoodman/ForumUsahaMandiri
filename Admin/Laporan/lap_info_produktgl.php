...
$pdf->Cell(6.5, 0.8, 'Nama Usaha', 1, 0, 'C');
...
$tglKe = $_GET ['tglKe'];
$no=1;
$query=mysql_query("SELECT * FROM tb_info_produk INNER JOIN tb_cat_produk on tb_info_produk.id_cat_produk=tb_cat_produk.id_cat_produk INNER JOIN tb_anggota ON tb_info_produk.user_id=tb_anggota.user_id WHERE disetujui='Belum' AND tb_info_produk.tanggal BETWEEN '$tglawal' AND '$tglKe' ORDER BY id_info_produk ASC ");
while($lihat=mysql_fetch_array($query)){
	$tanggal = $lihat ["tanggal"];
	$pdf->Cell(1, 0.8, $no , 1, 0, 'C');
	$pdf->Cell(6.5, 0.8, $lihat['nama_produk'],1, 0, 'C');
	...
	$pdf->Cell(6, 0.8, ''.substr($lihat['alamat_workshop'],0,25)."...", 1, 1,'C');
