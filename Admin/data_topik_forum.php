...
<div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        Data Topik Forum Forum Usaha Mandiri Kota Cimahi
...
$per_hal=10;
$jumlah_record=mysql_query("SELECT COUNT(*) from tb_forum_topik");
...
<div class="col-md-12">
 ...
 if (isset($_GET['st'])) {
    ...
    echo '<strong>Sukses!</strong> Data Topik berhasil di Update';
  echo "</div>";
    }elseif ($_GET['st']==2) {

        echo "<div class='alert alert-danger fade in'>";
    echo  "<a href=\"#\" class=\"close\" data-dismiss=\"alert\" aria-label=\"close\">&times;</a>";
    echo '<strong>Sukses!</strong> Data Topik Berhasil Di Hapus';
  echo "</div>";
...
<form action="cari_act8.php" method="get">
  <div class="input-group col-md-5 col-md-offset-7">
    <span class="input-group-addon" id="basic-addon1"><span class="glyphicon glyphicon-search"></span></span>
    <input type="text" class="form-control" placeholder="Cari Judul Topik di sini .." aria-describedby="basic-addon1" name="cari"> 
  </div>
</form>
<br/>
<table class="table table-hover"  style="border-color:black;">
  <tr>
    ...
    <th class="col-md-2">Judul Topik</th>
  ...
  if(isset($_GET['cari'])){
    ...
    $brg=mysql_query("SELECT * FROM tb_forum_topik INNER JOIN tb_forum_kategori ON tb_forum_topik.kategori_id=tb_forum_kategori.kategori_id INNER JOIN tb_user ON tb_forum_topik.user_id=tb_user.user_id WHERE judul_topik like='$cari'");
   }else if ($cari!='') {
    }else{
      $brg=mysql_query("SELECT * FROM tb_forum_topik INNER JOIN tb_forum_kategori ON tb_forum_topik.kategori_id=tb_forum_kategori.kategori_id INNER JOIN tb_user ON tb_forum_topik.user_id=tb_user.user_id limit $start, $per_hal");
    }
    $cek=mysql_num_rows($brg);
    if ($cek<1) {
      echo "<tr height=\"40\"><td colspan=\"6\" align=\"center\">Maaf Topik Forum tidak ada</td></tr>";       
    }
  $no=1;
  while($b=mysql_fetch_array($brg)){
    $tanggal = $b ["tanggal_topik"];
    ...
    <tr>
	   ...
	   <td><?php echo $b['judul_topik'] ?></td>
	    <td><?php echo $b['isi_topik'] ?></td>
	   <td><?php echo $b['dilihat'] ?></td>
       <td><?php echo ''.date( "d-m-Y", strtotime($tanggal) ); ?></td>
      <td>
       

         <a href="edit_topik_forum.php?id=<?php echo $b['id_topik']; ?>" class="btn btn-warning" title="Edit"><span class="glyphicon glyphicon-edit"></a>
        <a onclick="return confirm('Apakah anda yakin menghapus data ini ?');" href="../ClassFUM/proses_forum.php?aksi=hapustopik&id=<?php echo $b['id_topik']; ?>" class="btn btn-danger"title="Hapus"><span class="glyphicon glyphicon-trash"></a>
      </td>
    </tr>   
    <?php 
