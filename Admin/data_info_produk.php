...
$per_hal=10;
$jumlah_record=mysql_query("SELECT COUNT(*) from tb_info_produk where disetujui='Belum'");
...
<form action="cari_act4.php" method="get">
...
<table class="table table-hover"  style="border-color:black; ">
  ...
  if (isset($_POST['button'])) {
   $search = $_POST['search']; 
    ...
    $brg=mysql_query("SELECT * FROM tb_info_produk INNER JOIN tb_cat_produk on tb_info_produk.id_cat_produk=tb_cat_produk.id_cat_produk INNER JOIN tb_anggota ON tb_info_produk.user_id=tb_anggota.user_id where nama_produk like '%$cari%' and disetujui='Belum'");
     
    }else{
     $brg=mysql_query("SELECT * FROM tb_info_produk INNER JOIN tb_cat_produk on tb_info_produk.id_cat_produk=tb_cat_produk.id_cat_produk INNER JOIN tb_anggota ON tb_info_produk.user_id=tb_anggota.user_id where disetujui='Belum'");}
  ...
  while($b=mysql_fetch_array($brg)){
    ...
    <tr>
      ...
      <td>
        <a href="detail_produk.php?id=<?php echo $b['id_info_produk']; ?>" class="btn btn-primary" title="Detail"><span class="glyphicon glyphicon-briefcase"></a>
             <a class="trash btn btn-primary" data-id="<?php echo $b['id_info_produk']?>" href="#setujui" data-toggle="modal" title="Setujui"><span class="glyphicon glyphicon-ok-circle" aria-hidden="true"></a>
        <a  class="trash btn btn-danger" data-id="<?php echo $b['id_info_produk']; ?>" href="#tolak" data-toggle="modal"  title="Tolak"><span class="glyphicon glyphicon-remove-circle" aria-hidden="true"></a>
      </td>
    </tr>   
    <?php 
  }
...
<table><center>
<a href="Laporan/lap_info_produk_xls.php"><button type="button" class="btn btn-warning">Eksport Excel</button></a> || 
<a href="Laporan/lap_info_produk.php"><button type="button" class="btn btn-danger">Eksport PDF</button></a> ||
<a href="Laporan/lap_info_produk_doc.php"><button type="button" class="btn btn-info">Eksport Word</button></a>
    </table></center>
...
<script>
 ...
 $('.trash').click(function(){
    var id=$(this).data('id');
    $('#modalDelete').attr('href','../ClassFUM/proses_produk.php?aksi=verifikasi_tolak&id='+id);
    $('#modalDelete2').attr('href','../ClassFUM/proses_produk.php?aksi=verifikasi_setuju&id='+id);
})
  ...
  <div id="myModal1" class="modal fade" role="dialog">
          <div class="modal-dialog">
            ...
            <div class="modal-content">
              ...
              <form action="Laporan/lap_info_produktgl.php" method="GET" name="daftar" id="daftar">
              <div class="modal-body">
