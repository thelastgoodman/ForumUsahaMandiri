...
$per_hal=10;
$jumlah_record=mysql_query("SELECT COUNT(*) from tb_agenda where terlaksana='sudah'");
...
<div class="col-md-12">
 <?php 
  if (isset($_GET['st'])) {
    ...
    echo '<strong>Sukses!</strong> Agenda Berhasil Di Hapus';
  echo "</div>";
    }
  }
  ?>
  <a style="margin-bottom:10px" href="Laporan/lap_rekap_agenda.php" target="_blank" class="btn btn-default pull-right"><span class='glyphicon glyphicon-print'></span>  Cetak</a>
</div>
...
<table class="table table-hover"  style="border-color:black;">
  <tr>
    ...
    <th class="col-md-2">Isi Agenda</th>
    <th class="col-md-2">Gambar</th>
    <th class="col-md-2">Dilihat</th>
    <th class="col-md-2">Terlaksana</th>
  ...
  if(isset($_GET['cari'])){
    ...
    $brg=mysql_query("select * from tb_agenda where judul_agenda and terlaksana='sudah' like '%$cari%'");
   }else{
    $brg=mysql_query("select * from tb_agenda where terlaksana='sudah' order by tanggal_agenda desc limit $start, $per_hal");
  ...
  while($b=mysql_fetch_array($brg)){
    ...
    <tr>
      ...
      <td>
       ...
       <a onclick="return confirm('Apakah anda yakin menghapus agenda ini ?');" href="../ClassFK-PEL/proses_agenda.php?aksi=hapusagenda&id=<?php echo $b['id_agenda']; ?>" class="btn btn-danger"title="Hapus"><span class="glyphicon glyphicon-trash"></a>
      </td>
    </tr>   
    <?php 
  }
...
<ul class="pagination">     
    ...
    <!-- modal input -->
         ...
         <table><center>
<a href="Laporan/lap_rekap_agenda_xls.php"><button type="button" class="btn btn-warning">Eksport Excel</button></a> || 
<a href="Laporan/lap_rekap_agenda.php"><button type="button" class="btn btn-danger">Eksport PDF</button></a> ||
<a href="Laporan/lap_rekap_agenda_doc.php"><button type="button" class="btn btn-info">Eksport Word</button></a>
    </table></center>   
</section>
</div>
</div>
...
$('.trash').click(function(){
    var id=$(this).data('id');
     $('#modalsetujui').attr('href','../ClassFK-PEL/proses_produk.php?aksi=verifikasi_setuju&id='+id);
     $('#modaltolak').attr('href','../ClassFK-PEL/proses_produk.php?aksi=verifikasi_tolak&id='+id);
})
</script>
</body>
</html>